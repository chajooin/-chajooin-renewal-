...
const { uuidv4, s3Upload, pointFunc, consultingTalkSend, s3UploadResizeNew, calculatePercentage, s3Delete, getOptions, boardAlarmFunc, send, sendError, alarmFunc, itemDeleteFunc, kakaoApi, filterItems, logFunc, common, scd, frontNotCheck, adminScd, getFileName, sendSms, nice, niceDecode } = require('../../service/utils.js');
...
router.post('/list', async (req, res) => {
    ...
    let list = await models.BOARD_JEEIP_TB.scope(scopes).findAll({
        raw: true,
        order: orderList,
        where: whereList
    });
    ...
    for(var i = 1; i <= 2000; i++) {
        let test = {
            ...
            status: (i%2) + 1, // 상태  
            title: `지입${i} 교촌치킨 1일1배송/송탄~경기남부 오전7시~오후1시`, // 제목
            item: options?.item_option[i%options?.item_option?.length],
            go_sido: (i%2) === 1 ? "경북" : '경기', // 등록지역 시/도
            go_sigungu: (i%2) === 1 ? "예천군" : '시흥시', // 등록지역 시/군/구
            section: `운행구간입니다. ~~부터 운행지 ${i}번 까지`,
            unloading: options?.unloading_option[i%options?.unloading_option?.length],
            work: `${i%12}시부터 22시까지`,
            dayoff: "일휴무, 토일 휴무",
            pay: i,
            paytype: options?.paytype_option[i%options?.paytype_option?.length],
            offer: "제공사항입니다. 유류비, 상여금 등등 지원합니다.",
            jeeip_price: i+10,
            insurance_price: i+20,
            halbu_price: i+30,
            age: options?.age_option[i%options?.age_option?.length],
            passenger: "월~금 오후1시~6시",
        ...
        await models.BOARD_JEEIP_TB.create(
            test,
        );
    }
    /** 더미데이터 생성기 끝 */
...
router.post('/get', async (req, res) => {
    ...
    let row = await models.BOARD_JEEIP_TB.scope([
        'active',
        'likeCnt'
    ]).findOne({
        where: {
...
router.post('/insert', async (req, res) => {
    ...
    let row = "";
    if(idx) {
        row = await models.BOARD_JEEIP_TB.findOne({
            where: {
                idx: idx,
                u_idx: user?.idx
            }
    ...
    console.log('boardStatusConsts', boardStatusConsts);
    // console.log(option?.usage_option?.includes(usage));
    // console.log(option?.option1?.includes(type));
    // console.log(option?.axis_option?.includes(axis));
    // console.log(option?.color_option?.includes(color));
    ...
    if(!go_sido) { sendError(req, res, {code: 1, msg: '출근지 지역을 선택해주세요.'}); return; }
    if(!go_sigungu) { sendError(req, res, {code: 1, msg: '출근지 지역을 선택해주세요.'}); return; }

    if(!section) { sendError(req, res, {code: 1, msg: '운행구간을 입력해주세요.'}); return; }
    if(unloading?.length < 1) { sendError(req, res, {code: 1, msg: '상차 형태를 선택해주세요.'}); return; }
    if(unloading_2?.length < 1) { sendError(req, res, {code: 1, msg: '하차 형태를 선택해주세요.'}); return; }
    ...
    if(!option?.paytype_option?.includes(paytype)) { sendError(req, res, {code: 1, msg: '지급방식을 선택해주세요.'}); return; }
    if(!jeeip_price) { sendError(req, res, {code: 1, msg: '지입료를 입력해주세요.'}); return; }

    if(!option?.age_option?.includes(age)) { sendError(req, res, {code: 1, msg: '나이제한을 선택해주세요.'}); return; }
    ...
    let updateList = {
        ...
        unloading_2: unloading_2?.join(","),
        work,
        dayoff,
        pay: pay || 0,
        paytype,
        offer,
        jeeip_price: jeeip_price || null,
        insurance_price: insurance_price || null,
        halbu_price: halbu_price || null,
        age,
        passenger,
        maker,
        car,
    ...
    if(file_path?.base) {
        let urls = await s3Upload(file_path?.base, 'board', `세금계산서_${user?.idx}_${moment().unix()}`, file_path?.ext);
        if(urls) { updateList.file_path = urls; }
    }
    ...
    try {
        if(row) {
            ...
            row = await models.BOARD_JEEIP_TB.create(
                updateList,
            );
        }
    } catch (error) {
...
router.post('/consulting', async (req, res) => {
    ...
    /* 신청내역 검사 */
    if(type*1 !== 1 && consulting?.length > 0) { // 이미 전화상담 내역이 있는경우 중단
        send(req, res, rt);
        return;
    }
